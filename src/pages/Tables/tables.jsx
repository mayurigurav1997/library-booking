...
const Tables = () => {
  ...
  const handleSlotBack = () => {
    ...
    var filterBookedSeats = userData?.flatMap((user) =>
      user?.bookingStatus
        .filter(
          (booking) =>
            booking.slot == slotsTimings[index - 1] &&
  ...
  function handleSeatSelecting(i) {
        ...
        user?.bookingStatus
          .filter(
            (booking) =>
              booking.slot &&
              booking.date == selectedDate &&
              (booking.seatId || booking.seatSelected),
          )
          .map((booking) => booking.seatSelected),
      )
    let slotBooking = userData
      ?.filter((user) => user.name === userName)
      .flatMap((user) =>
        user?.bookingStatus
          .filter(
            (booking) =>
              booking.slot == selectedSlot && booking.date == selectedDate,
          )
          .map((booking) => booking?.seatSelected),
      )
    console.log(copyUserData, 'copyUserData')
    ...
    if (totalBooking.length >= 3) {
      alert(
        "You have already selected 3 slots for this day, Can't select slot for this day now",
      )
      setSelectedButtonId(null)
      ...
      const updatedCopyUserData = copyUserData.map((user) =>
        user.name === userName
          ? {
              ...user,
              bookingStatus:
                user.bookingStatus.length > 0
                  ? user.bookingStatus.some(
                      (book) =>
  ...
  const handleNext = () => {
    ...
    let paymentData = copyUserData
      ?.filter((user) => user.name === userName)
      .flatMap((user) =>
        user?.bookingStatus
          .filter(
            (booking) => booking.slot && booking.date && booking.seatSelected,
          )
          .map((booking) => booking),
      )
  ...
  return (
    ...
    >
      ...
      >
        <Typography3 variant="h5" sx={{ mb: 3 }}>
          Select your tables
        </Typography3>
        <Typography4 variant="h6" sx={{}}>
          Select date
        </Typography4>
        <Box sx={{ mb: 3 }}>
        ...
        >
          <Button
            ...
            disabled={
              (selectedDate ? false : true) ||
              slotsTimings[index] == '4:00AM - 6:00AM'
            }
          >
            Previous
          </Button>
          ...
          <Button
            ...
            disabled={
              (selectedDate ? false : true) ||
              slotsTimings[index] == '8:00PM - 10:00PM'
            }
          >
            Next
          </Button>
      ...
      <Typography4 variant="h6" sx={{ mb: 2 }}>
        Select tables
      </Typography4>
      {selectedSlot ? (
        <Box
        ...
        >
          ...
          {Row1.map((i) => (
            <Button
              variant={
                selectedButtonId == i || activeButton == i
                  ? 'contained'
                  : 'outlined'
              }
              key={i}
