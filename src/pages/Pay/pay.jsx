...
const Pay = () => {
  ...
  const handleNext = () => {
    ...
    const filteredData = copyUserData.map((user) => ({
      ...user,
      bookingStatus: user.bookingStatus.map((book) => ({
        ...book,
        seatId: book.seatSelected ? book.seatSelected : book.seatId,
        seatSelected: '',
      })),
    }))
  ...
  return (
    ...
    >
      ...
      >
        <Typography3 variant="h6" sx={{ textAlign: 'center', mb: 2 }}>
          Your Selection
        </Typography3>
        <TableContainer
          component={Paper}
        ...
        >
          <StyledTable aria-label="simple table">
            <TableHead>
              <TableRow>
                <StyledTableHeadingCell align="left">
                  Day and time
            ...
            <TableBody>
              {paymentData.map((row) => (
                <TableRow key={row.seatSelected}>
                  <StyledTableCell align="left">
                    {moment(row.date).format('Do MMMM YYYY')} {row.slot}
                  </StyledTableCell>
                  <StyledTableCell align="left">
                    {row.seatSelected}
                  </StyledTableCell>
